...
**Demostración dinámica acelerada (`openspec/changes/add-accelerated-sensor-demo/`, 2026-09-17):** capacidad nueva, no parte del balance 6/6 anterior. ADR-0012 (controlador local opcional) aceptado vía PR #201, solo como decisión de arquitectura — todavía no implementado. Entrega 1 (preparación y worker CLI, `scripts/demo_simulation/`) implementada y verificada: `prepare` genera un sensor exclusivo `demo-<id>` con historial sintético previo al período a reproducir y manifiesto atómico; `run` ejecuta la secuencia ingesta→verificación→pronóstico→confirmación un día calendario por paso contra el backend real, deteniéndose (`status=blocked`) ante cualquier fallo o resultado no confirmable, sin reintentar POSTs ni interpretar un timeout como cancelación. 37 tests nuevos (`tests/demo_simulation/`) contra un backend real (`uvicorn` en un hilo, HTTP real, sin `TestClient`) y registro MLflow aislado (sqlite en directorio temporal), incluyendo cinco pasos consecutivos con verificación de causalidad, orden HTTP, ausencia de días futuros y preservación de archivos de sensores ajenos. Entregas 2-4 (control local con lock/pausa, servicio HTTP, UI, Docker) pendientes; no se declara recuperación automática de fallos ni exclusión entre procesos, que corresponden a la entrega 2.
