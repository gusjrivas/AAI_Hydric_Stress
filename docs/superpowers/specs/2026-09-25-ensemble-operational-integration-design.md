...
**Detección de modo:** un sensor/horizonte con `ensemble_manifest.json` presente está en modo ensamble para ese horizonte; su ausencia preserva exactamente el comportamiento single-model actual (sin cambios, sin flag de configuración nuevo).

### 3.2 Carga: `load_ensemble_bundle`

Nuevo módulo `src/predictive_modeling/ensemble_bundle.py`. `load_ensemble_bundle(bundle_root: Path, *, sensor_id: str, horizon: int) -> EnsembleBundle | None` (devuelve `None`/lanza si no hay manifiesto — el llamador decide). Pasos:

1. Leer y validar `ensemble_manifest.json` (`format_version`, `mode=="ensemble"`, `policy_version` en un conjunto soportado, `sensor_id`/`horizon_days` coinciden con los parámetros).
...
`predict_ensemble_bundle(ensemble: EnsembleBundle, dataframe, *, sensor_id, units, as_of_date) -> dict`. Llama a `predict_operational_bundle` (sin modificar) una vez por familia, sobre el mismo `dataframe`/`as_of_date` — el corte causal ya existente en esa función se reutiliza sin cambios para cada componente. Con los 3 resultados, deriva **cuatro campos separados, nunca combinados entre sí**:

- `probabilidad_combinada`: promedio de las 3 probabilidades calibradas, pesos fijos 1/3 (`manifest["weights"]`).
- `decision_binaria_combinada`: `probabilidad_combinada >= decision_threshold` — el mismo `decision_threshold` ya validado idéntico entre las 3 (paso 3.2.3), no uno nuevo inventado.
- `votos_positivos`: cuenta de componentes cuyo **propio** `alert` (de su propio `decision_threshold`) es verdadero.
...
`producer_emission.py::emit_forecasts` — antes de llamar a `load_operational_bundle` para un horizonte dado, comprobar si existe `ensemble_manifest.json` en esa ruta. Si existe: usar `load_ensemble_bundle`/`predict_ensemble_bundle`; si no: camino actual, sin cambios. Ambos caminos conviven en la misma función, sin pipeline paralelo ni backend duplicado.

Cuando el modo ensamble está configurado pero incompleto/inválido: `SlotSeed(status="unavailable", reason_code="ensemble_component_missing:<family>"` o `"ensemble_contract_mismatch:<campo>"` o `"ensemble_temporal_inadmissible"`)`. Nunca se devuelve un resultado simulado como si fuera real.
...
- Compatibilidad: sensores/horizontes sin `ensemble_manifest.json` producen exactamente la misma respuesta que hoy (test de regresión explícito).
...
Se entrega como documento separado (`docs/design/ensemble-real-enablement-plan.md`), sin ejecutar nada, cubriendo:
...
- Ambigüedad resuelta: "decisión binaria por su umbral" (encargo original) se interpretó como el umbral ya existente (0.5, común a las 3 familias), nunca uno nuevo — documentado explícitamente en 3.3.
