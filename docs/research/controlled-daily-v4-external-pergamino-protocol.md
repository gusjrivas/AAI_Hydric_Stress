...
- Combinación propia de probabilidades (`SoftVotingClassifier`, `src/experiment_runner/controlled_daily_v4/models.py`), construida únicamente con las tres familias anteriores — **no** `sklearn.ensemble.VotingClassifier`: ese estimador solo admite un único vector de `sample_weight` propagado a los tres sub-estimadores, y por lo tanto no puede representar una combinación mixta donde cada familia elige de forma independiente su propio modo de balanceo (sección 7.1). La implementación clona y ajusta cada base por separado y promedia `predict_proba` con los pesos fijos de abajo, alineando explícitamente las columnas de probabilidad contra `classes_` del ensamble.
...
2. Concatenar las predicciones OOF en orden temporal (los tres `outer_val` son disjuntos, cronológicamente ordenados y, en esta implementación, calendario-adyacentes entre sí — por ejemplo, un `outer_val` puede terminar el 13/04 y el siguiente comenzar el 14/04. La discontinuidad de `gap=3` no está entre un `outer_val` y el siguiente: está **dentro** de cada fold, entre el fin de su propio `outer_train` y el comienzo de ese mismo `outer_val` — ver sección 6. Esa purga intra-fold evita fuga causal entre entrenamiento y validación de un mismo fold; la regla que sí separa los tres segmentos entre sí a efectos del bootstrap es otra, predeclarada, no basada en una distancia calendario — ver sección 8, paso 5, y la nota siguiente).
...
**Justificación de la variante no circular:** la variante circular (Politis & Romano) reduciría el sesgo de muestreo en los extremos de cada segmento, pero para lograrlo debe envolver el final de un segmento con su **propio** comienzo al construir un bloque — es decir, tratar como temporalmente adyacentes el último y el primer día de ese mismo segmento, que en la serie real no lo son (el segmento no es circular). Esa vecindad artificial contradice el diseño segment-aware exigido por este protocolo (paso 5, arriba): cada `outer_val` es un segmento de remuestreo independiente, y ningún bloque puede cruzar el límite entre dos segmentos distintos — esto es una regla predeclarada del remuestreo, no una consecuencia de que existiera una separación calendario entre ellos. De hecho, en esta implementación los tres `outer_val` quedan prácticamente consecutivos en el calendario (ver nota del paso 2, arriba) y aun así se tratan como segmentos disjuntos a efectos del bootstrap. La purga por `gap=3` es un mecanismo distinto, interno a cada fold (fin de `outer_train` → comienzo de su propio `outer_val`, sección 6): evita fuga causal dentro de un mismo fold, y no crea ni pretende crear una discontinuidad entre los `outer_val` de folds sucesivos.
...
`PRECONDITION_FOR_EXECUTION`, ya satisfecha para la ruta de tests/CI con datos sintéticos; pendiente de confirmarse también para la corrida científica real (ver nota final):

- Versiones exactas de Python, NumPy, pandas, SciPy, PyArrow y scikit-learn fijadas en `docker/experiment-v4/constraints.txt`, construidas en un contenedor limpio con digest de base fijado (`docker/experiment-v4/Dockerfile`), y validadas ejecutando allí la suite de tests con datasets sintéticos (`pip check` en verde, dos veces).
- `environment.capture_environment()`/`environment.validate_environment()` (`src/experiment_runner/controlled_daily_v4/environment.py`) capturan las versiones efectivamente importadas ANTES de ajustar cualquier modelo y las contrastan contra `constraints.txt` — en modo `scientific` (default de la CLI), una discrepancia aborta la corrida sin llegar a entrenar (hallazgo H-04). En modo `synthetic` esta validación no bloquea nada.
- `pyproject.toml` sigue declarando solo pisos mínimos (`scikit-learn>=1.4`, `pandas>=2.0`, `pyarrow>=14.0`) para el proyecto en general; el entorno estrictamente fijado es el dedicado de `docker/experiment-v4/`, no un reemplazo del `pyproject.toml` raíz.
- **Alcance de lo ya validado:** cubre exclusivamente reproducibilidad para tests/CI con datos sintéticos. La ejecución real de la Etapa A sobre Pergamino no se realizó todavía y exige, además, congelar el commit exacto usado en esa corrida (`repository_state.commit` del manifiesto de provenance, hoy `PENDING_BEFORE_EXECUTION`) — ver `code_identity.capture_code_identity()`, que además de la versión del entorno registra el SHA de código y si el árbol estaba limpio o modificado, con manejo explícito para cuando no hay `.git` disponible (ejecución dentro del contenedor).
