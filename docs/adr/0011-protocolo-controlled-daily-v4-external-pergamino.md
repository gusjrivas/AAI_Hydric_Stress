...
**Actualización (2026-09-15):** el mecanismo de software que hace cumplir este criterio -- el ledger transaccional de protección del holdout (`src/experiment_runner/controlled_daily_v4/holdout_ledger.py`), la admisibilidad de C sobre el veredicto de B (`admissibility.check_stage_c_admissibility`) y el runner de la Etapa C (`stage_c_runner.py`) -- están implementados e integrados, verificados exclusivamente con pruebas sintéticas (ver `docs/research/controlled-daily-v4-stage-b-c-decisiones-pendientes.md` para el detalle completo de la identidad del holdout, la secuencia de apertura y los tres estados del registro). Esto NO constituye la apertura real del holdout: ninguna ejecución científica real se realizó, y no existe ningún ledger científico inicializado en este repositorio.
