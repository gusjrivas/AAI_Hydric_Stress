# Complemento condicional de robustez

## Why

Implementar y validar auxiliary_robustness_v1 solo si S=REQUIRED; ejecutar luego únicamente con autorización científica específica.
...
Gate adicional obligatorio: S REQUIRED AND sc-01/sc-02 PASS AND implementation approved; scientific execution additionally requires validated runner and authorization S; NOT_REQUIRED/UNRESOLVED blocks execution


- `src/experiment_runner/scientific_auxiliary/auxiliary_robustness_v1.py`
- `tests/test_auxiliary_robustness_v1.py`
- Este directorio OpenSpec: tasks.md y registros de revisión nuevos identificados.
- Gobernanza externa futura: evidence/governance/<campaign-id>/sc-10-aux-robustness/, solo con autorización de campaña.

- Salidas auxiliares en evidence/auxiliary/sc-10-aux-robustness/, nunca A/B/C; solo con autorización separada.
...
Ver tasks.md. Requisitos: SC-GOV-024.
- `tests/test_auxiliary_robustness_v1.py` (nuevo, todavía pendiente)

## Evidencia esperada

- `auxiliary/S/review.json`: SC-GOV-024
...
Necesidad S=REQUIRED anterior a ejecución, diseño congelado respetado, fixtures aprobados y evidencia auditada. Si NOT_REQUIRED: solo decisión motivada, sin implementación.
