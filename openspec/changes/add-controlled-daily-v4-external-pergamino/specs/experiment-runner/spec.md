...
> Estado de este delta: ~~**propuesta, sin implementación de código todavía** (`PROTOCOL_ONLY`, ver `docs/research/controlled-daily-v4-external-pergamino-manifest.yaml`). Ninguno de los escenarios siguientes está implementado ni ejecutado; se documentan como el contrato verificable que una futura implementación deberá cumplir.~~ **Actualización (2026-09-13):** el *change* `implement-controlled-daily-v4-stage-a` (mergeado; correcciones en PR #189/#190) implementó código y pruebas sintéticas para los escenarios de Etapa A de este delta (etiquetado estricto, fronteras temporales por etapa, historia causal cruzando etapas, invariante de nested CV, selección por MCC global con margen práctico). Ningún escenario fue verificado con ejecución real sobre Pergamino todavía — toda verificación existente es exclusivamente sintética (ver `docs/seguimiento-tareas.md`). Los escenarios de "Compuerta real de validación temporal" (Etapa B) y "El holdout no admite ajustes posteriores" (Etapa C), más abajo, siguen sin ningún código asociado — ver `openspec/changes/implement-controlled-daily-v4-stage-b-c/` para su propuesta de implementación. Este archivo no se modifica en su contenido normativo (los Given/When/Then permanecen como el contrato original de este *change*, ya mergeado); solo se corrige esta nota de estado, que había quedado desactualizada. No reemplaza ni modifica el requirement vigente de `controlled_daily_v3` en `openspec/specs/experiment-runner/spec.md`.
...
- **THEN** cada una de esas métricas queda indefinida (matemáticamente `NaN`) y nunca se reporta como 0; en el artefacto JSON serializado esa indefinición se representa mediante el envelope explícito `{"value": null, "status": "undefined", "undefined_reason": "<motivo>"}` (protocolo, sección 12) — el artefacto nunca contiene el token literal `NaN` — y ese conjunto no puede producir `CANDIDATE_VALIDATED` en la Etapa B

**Corrección (2026-09-13):** el THEN de este escenario decía "se registra como `NaN` explícito", en aparente contradicción con el protocolo (sección 12: "ningún artefacto... contiene los tokens no estándar `NaN`") y con la implementación (`allow_nan=False`, `artifacts.py`). Se corrige la redacción para distinguir la convención matemática (`NaN` en el cálculo en memoria) de su representación serializada (envelope JSON) — no se modifica ningún criterio de aceptación: el requisito siempre fue que la indefinición nunca se reporte como 0 ni se pierda su motivo, y ambas formulaciones son consistentes con eso.

Estado de implementación por escenario: los seis escenarios de Etapa A de este delta cuentan con código y verificación sintética (ver nota de estado al inicio de este archivo). Los dos escenarios de Etapa B y C, arriba, no tienen implementación asociada — ver `openspec/changes/implement-controlled-daily-v4-stage-b-c/` para su propuesta.
