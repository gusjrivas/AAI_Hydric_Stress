...
- [x] Agregar tests unitarios y de integración con fixtures sintéticas cubriendo causalidad, invariante temporal, grillas, selección, congelamiento, serialización, CLI y ausencia de MLflow — **67 tests, exclusivamente con datos sintéticos**, en la implementación inicial de este *change*. Tras las correcciones de la sección siguiente ("Correcciones derivadas de la revisión técnica dirigida"), la suite dirigida de este mismo *change* asciende a **144 tests** (ver `proposal.md`, sección "Verificación") — cifra corregida el 2026-09-13 para no contradecir el resto de este archivo, que ya documenta esas correcciones como completadas. Auditorías post-merge posteriores (H-01 a H-06, *changes* separados) llevaron la suite `controlled_daily_v4` a 231 tests — ver `docs/seguimiento-tareas.md` para esa cifra, no reproducida aquí.
