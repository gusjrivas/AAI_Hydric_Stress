...
`BLOCKED`, `NOT_APPLICABLE`, `PENDING`. Los identificadores de esta tabla se
...
| **SC-GOV-007** | PENDING | El artefacto esperado `workflow-events.jsonl` no existe en el repositorio. `agent-capabilities.json` documenta separación de roles observada, pero no es el registro de eventos del DAG ni sustituye los manifiestos de revisión por SHA |
...
| **SC-GOV-019** | PENDING | El artefacto esperado `checkpoints.json` no existe. El estado se lleva en `current-execution-checkpoint.md` y en registros JSON de campaña, que no son ese artefacto |
...
Resumen mecánico: PASS 0; PASS_WITH_LIMITATIONS 7 (SC-GOV-001, 003, 004, 005,
006, 017, 020); BLOCKED 13 (SC-GOV-002, 008, 009, 010, 011, 012, 013, 014, 015,
016, 018, 022, 025); NOT_APPLICABLE 3 (SC-GOV-021, 023, 024); PENDING 2
(SC-GOV-007, 019). Total 25.
