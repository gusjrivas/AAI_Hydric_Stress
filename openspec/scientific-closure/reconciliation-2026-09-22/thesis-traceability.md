...
| 2.2 | `temporal-contract-check.json`; `controlled_daily_v4/features.py::validate_continuous_daily_calendar` | Contrato de ocho features idéntico en las tres etapas (verificado por el artefacto); target observado a t+3 no imputado, y **v4 no imputa entradas: aborta ante huecos del calendario diario** (verificado en código, no en el artefacto) | «El contrato de variables y la definición del objetivo se mantuvieron idénticos en las tres etapas, y no se introdujo fuga temporal» | Verificación por atestación retrospectiva reconciliada con la evidencia, no por registro emitido durante la corrida | Tabla: las ocho variables con su origen (`soil_moisture`, `RH2M`, `ALLSKY_SFC_SW_DWN`, `lag1..3`, `roll_mean_3`, `roll_mean_7`) | **Corregido tras el hallazgo `F-01` de la auditoría de la reconciliación 2026-09-22:** la fila original atribuía a `temporal-contract-check.json` una verificación de «imputación causal sólo de entradas» que el artefacto no contiene (sus cinco `leakage_checks` son de fronteras temporales, contaminación de P20 y contrato de features). La propiedad real —v4 no imputa, aborta— es más estricta, y su fuente correcta es el código del runner, no ese artefacto |
...
| 2.8 | `auxiliary/{R,N,S}/review.json`; GD-12 | Decisión de suficiencia por complemento, **predeclarada** antes de la ejecución (GD-12, 2026-09-19); su **crítica y auditoría independientes** (RB-03) son **posteriores** a la campaña, del 2026-09-22, sobre complementos que no se activaron | «El alcance de los complementos se delimitó antes de ejecutar A/B/C/H; esa delimitación fue criticada y auditada de forma independiente después de la campaña, sobre complementos no activados» | `NOT_REQUIRED` limita la afirmación; no significa que el complemento sea innecesario para el problema; **la revisión independiente no fue previa a la ejecución** y no debe presentarse como tal | Ninguna | **Corregido 2026-09-22 (instrucción del responsable sobre PR #212):** la fila anterior decía «anterior a la ejecución y auditada de forma independiente», que conflacionaba la fecha de la decisión (2026-09-19, anterior) con la de su revisión (2026-09-22, posterior a A/B/C/H del 2026-09-21) |
...
| 3.4 | `code_version.json`, `environment.json`, `resolved_config.json`, `input_hashes.json` | Identidad reproducible: commit ejecutable `214735e…`, imagen `sha256:55bc923e…`, semillas 42 y 20250109, huella del conjunto `8062605b…` | «Cada corrida registra commit, imagen, configuración, semillas, hashes de datos y entorno» | La identidad **documental** es distinta de la **ejecutable** y debe declararse aparte. **La inspección de la imagen desde este entorno tuvo un fallo inicial (sesión 2026-09-19/20, EXT-02: runtime de contenedores inalcanzable desde la distro WSL) y una verificación posterior exitosa desde la misma distro Ubuntu/WSL (sesión 2026-09-21): el daemon Docker respondió y ejecutó la imagen aprobada `sha256:55bc923e…b297af`**, que es la que produjo A, B y C | Tabla: identidad de la campaña en una sola vista | **Corregido 2026-09-22 (instrucción del responsable sobre PR #212):** la fila anterior decía «la imagen no fue inspeccionable desde el entorno de validación Linux», que era cierto sólo para la sesión inicial y quedó superado |
| 3.5 | `docker/experiment-v4/Dockerfile` y `constraints.txt` | Imagen inmutable con dependencias fijadas, base fijada por digest | «El entorno de ejecución se congeló en una imagen con dependencias fijadas por constraints» | `docker/experiment-v4/build.ps1` existe en esta rama y no en `main`. **Corregido 2026-09-22 (hallazgo M-03 de la auditoría Codex RB-05, `rb05-audit-preparation-2026-09-22/reviews/review-audit-codex-round1-FAIL.md`):** la fila anterior decía «el runtime de contenedores no es alcanzable desde la distro WSL usada para validar», contradiciendo la fila 3.4 inmediatamente anterior, ya corregida el mismo día. Esa afirmación fue cierta sólo para la sesión inicial (2026-09-19/20, EXT-02); la sesión del 2026-09-21 (`current-execution-checkpoint.md:27`) verificó que el daemon Docker responde y ejecuta la imagen aprobada desde la misma distro Ubuntu/WSL, que es la que produjo A, B y C | Ninguna | Ninguno |
...
| 3.12 | **Nueva 2026-09-22, hallazgo M-01/RK-20 de la auditoría RB-05** (`openspec/scientific-closure/decisions.md` GD-38, GD-40; `openspec/scientific-closure/risks.md` RK-20) | El diseño predeclarado exige auditoría independiente de cada etapa **antes** de autorizar la siguiente (Gate A → B, Gate B → C). La ejecución efectiva del 2026-09-21 no lo cumplió: `changes.json` y las marcas de tiempo de sistema de archivos de los `gate-review.json` muestran que B se ejecutó antes de que existiera el veredicto de auditoría de A, y C antes del veredicto de auditoría de B | «El protocolo predeclara una secuencia de gates auditados; la ejecución de la campaña de 2026-09-21 no siguió esa secuencia de gobernanza como estaba diseñada, un incumplimiento verificado y aceptado administrativamente como desviación permanente» | **Debe distinguirse explícitamente diseño de ejecución.** No es un defecto del diseño del protocolo (SC-GOV-012/014 exigen correctamente la secuencia); es un defecto de cómo se ejecutó y registró esa secuencia. No invalida los resultados numéricos de B y C, que se conservan como evidencia real | Figura recomendada: línea de tiempo real de A/B/C (ejecución) contrastada contra la secuencia de gates exigida (diseño) | Clasificar como **desviación de gobernanza experimental** en el capítulo 3; no presentar como fallo del diseño del protocolo ni como fallo numérico de los resultados |
...
| R.2 | Etapa B (**evaluación retrospectiva exploratoria**, no validación confirmatoria — ver 3.12) | `CANDIDATE_VALIDATED` por no inferioridad; Δ MCC 0,0866 con IC [−0,0146; 0,2092] | «El candidato resultó no inferior a la persistencia causal dentro del margen predeclarado, en una evaluación retrospectiva exploratoria» | **El intervalo incluye el cero**; la precisión del candidato (0,667) es inferior a la de la persistencia (0,723). **Corrección 2026-09-22:** la auditoría independiente de la Etapa A se registró *después* de que B ya hubiera corrido (M-01/RK-20); **prohibido** describir este resultado como «validación confirmatoria» gobernada por el protocolo secuencial | Tabla: candidato contra persistencia en 2023 | Destino: capítulo 4 |
| R.3 | Etapa C (**evaluación retrospectiva exploratoria**, no validación confirmatoria — ver 3.12) | Δ MCC 0,0913 con IC [0,0227; 0,1784], que excluye el cero; `episode_recall` 0,900 | «Sobre el holdout final, en una evaluación retrospectiva exploratoria, el candidato superó a la persistencia en MCC, con el intervalo pareado excluyendo el cero» | ~24 unidades efectivas; sin corrección por multiplicidad; **una sola serie**. No es superioridad general. **Corrección 2026-09-22:** la auditoría independiente de la Etapa B se registró *después* de que C ya hubiera corrido (M-01/RK-20); **prohibido** describir este resultado como «holdout confirmatorio exitoso» o equivalente. Una confirmación futura exige una campaña nueva sobre datos no utilizados previamente, no una relectura de este holdout | Tabla: candidato contra persistencia en 2024–2025 | Destino: capítulo 4 |
