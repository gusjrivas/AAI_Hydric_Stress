...
**Reutilización de las tres familias de `controlled_daily_v4.models` por la integración operativa del ensamble (2026-09-25, rama `feat/ensemble-operational-integration`, HU6/`architecture-integration` + `predictive-modeling`):** `build_estimator`/`fit_estimator` (regresión logística, Random Forest, HistGradientBoosting) se reutilizan sin modificación desde `src/predictive_modeling/ensemble_bundle.py` para construir un contrato de ensamble operativo (política `ensemble_agreement_v1`, tres votos + probabilidad media calibrada). Esto es reutilización de código, no una ejecución experimental de HU7/HU8: se verificó exclusivamente con datos sintéticos (`tests/test_ensemble_bundle_real_families.py`, `tests/test_ensemble_bundle.py`), nunca con los CSV de Pergamino/Balcarce ni con el dataset formal `melchor_romero_2024_consolidado`. No se encontraron artefactos v4 reales (modelo+calibrador serializados) en lo inspeccionado en ninguna ubicación de este repositorio ni de la infraestructura asociada (detalle en `docs/superpowers/specs/2026-09-25-ensemble-operational-integration-design.md`, sección 2); esto no descarta que existan en almacenamiento no inspeccionado desde esta máquina. Ningún archivo de `src/experiment_runner/controlled_daily_v4/` (`freezing.py`, `tuning.py`, `models.py`, los *stage runners*) se modificó para esta integración.
