...
- **Etapa B** (reentrenamiento del candidato congelado y compuerta temporal de validación sobre 2023, `src/experiment_runner/controlled_daily_v4/stage_b_runner.py`, más el contrato de transferencia A→B): **implementada y verificada sintéticamente** en el PR #193 (rama `feat/controlled-daily-v4-stage-b`); su **integración a `main`** queda **pendiente** mientras ese PR siga abierto.
- **Ejecuciones científicas reales de A y/o B, y sus resultados cuantitativos** (sobre los CSV de Pergamino/Balcarce): **pendientes** — ninguna se realizó todavía; esta sección no las anticipa ni las presenta como obtenidas.
- **Etapa C y su ledger de holdout**: **sin implementar**, fuera de alcance de este encargo (Decisiones 2 y 3, ambas bloqueadas — `docs/research/controlled-daily-v4-stage-b-c-decisiones-pendientes.md`).

**Nota de resolución (2026-09-15):** hasta esta actualización, la tarea de reflejar aquí la Etapa B quedaba deliberadamente sin marcar (`openspec/changes/implement-controlled-daily-v4-stage-b-c/tasks.md`), señalando que la tarea equivalente de la Etapa A (`openspec/changes/implement-controlled-daily-v4-stage-a/tasks.md`) dejaba esa misma actualización pendiente de una ejecución científica real, y que actualizar solo B sin resolver A sería inconsistente con ese precedente. Encargo explícito (2026-09-15): documentar una capacidad ya implementada e integrada (A) o implementada y verificada sintéticamente en un PR abierto (B) no depende de que exista una ejecución científica real — el mismo criterio ya usado en este documento para otras capacidades de HU7 (por ejemplo, la retroalimentación humana dentro del plan experimental formal, más arriba: "implementado e integrado", con su evaluación cuantitativa formal declarada aparte como diseñada pero no ejecutada). Esta nota resuelve la contradicción para A y B a la vez, sin reescribir ningún resultado histórico: las tablas y hallazgos cuantitativos de las secciones siguientes (`controlled_daily_v3`) no se modifican, y esta sección no afirma ninguna ejecución científica de A o B, ni resultado alguno de la Etapa C (sin implementar) o de su ledger, que no haya ocurrido.
