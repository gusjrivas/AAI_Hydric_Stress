...
**Actualización (2026-09-05):** una segunda auditoría metodológica encontró que este orden, aunque correcto para imputación y umbral, seguía dejando una fuga más sutil: la etiqueta objetivo (`add_stress_label`, con `shift(-horizon_days)`) se calcula sobre la concatenación de train y test ya imputados, y las últimas `horizon_days` filas de train reciben así una etiqueta derivada de una fecha que cae dentro del período de evaluación. La implementación vigente purga el entrenamiento por fecha objetivo: cada fila contiene `target_timestamp = timestamp + horizon_days` y solo puede pertenecer al entrenamiento cuando `target_timestamp < cutoff`. De este modo ninguna etiqueta de entrenamiento puede depender de una fecha perteneciente al período de evaluación. La capacidad `purge_target_horizon` (`data_quality.splitting`, ver `openspec/specs/data-quality/spec.md`) permanece disponible y testeada en `data-quality`, pero el orquestador actual utiliza el criterio explícito por fecha objetivo descrito arriba. Se agregó además `gap=horizon_days` a la validación cruzada temporal de selección de modelo (`select_best_candidate`, `openspec/specs/predictive-modeling/spec.md`), para que ningún fold de entrenamiento incluya un objetivo que se solape con su fold de validación. Verificado con `tests/test_architecture_integration_pipeline.py::test_changing_a_test_period_value_does_not_change_the_selected_model_or_its_training_data`, que confirma con `pd.testing.assert_frame_equal` que modificar un valor de test no altera el conjunto de entrenamiento resultante. Ver `docs/research/hu8-analisis-resultados.md`, sección 12, para el impacto sobre los resultados ya reportados.
