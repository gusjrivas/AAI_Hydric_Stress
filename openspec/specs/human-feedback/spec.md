...
**Lectura *fail-closed* (T-01):** el marcador canónico de que una versión de modelo *declaró* linaje son los parámetros indexables ya persistidos en esa versión (`recalibration_id`, `source_model_id`, `successor_model_id`, `dataset_fingerprint`), no la mera presencia descargable del artefacto opcional. Si una versión no tiene esos parámetros, `load_recalibration_lineage`/`list_recalibration_lineage` la tratan como historial legítimamente ausente y devuelven/omiten `None` — comportamiento retrocompatible. Si una versión sí los tiene, cualquier problema para reconstruir su linaje (artefacto ausente, error de descarga, JSON inválido, semántica inválida, o inconsistencia entre el artefacto y esos mismos parámetros) levanta `LineageValidationError` con contexto (versión y `run_id`, sin datos sensibles) en vez de degradar silenciosamente a `None`; `list_recalibration_lineage` propaga ese error en vez de devolver una cadena parcial que aparente estar completa.
