...
- **GIVEN** una versión que sí tiene esos parámetros (declaró linaje), pero su artefacto está ausente, falla al descargarse, tiene contenido ilegible, JSON inválido, estructura inválida, una `lineage_version` no soportada, incumple alguna invariante semántica, es inconsistente con los parámetros ya persistidos en esa misma versión, o el parámetro MLflow `lineage_version` no coincide exactamente con el `lineage_version` del artefacto (incluida su ausencia cuando el artefacto no es `LINEAGE_VERSION_1`)
- **WHEN** se recupera su linaje (`load_recalibration_lineage`) o se reconstruye la cadena completa (`list_recalibration_lineage`)
- **THEN** falla explícitamente (`LineageValidationError`, con la versión del modelo y el `run_id` afectados en el mensaje) en vez de devolver `None` o una cadena parcial que aparente estar completa

#### Scenario: `dataset_sha256` y el `DataFrame` usado provienen de la misma instantánea de bytes

- **GIVEN** una recalibración sobre el dataset consolidado de un sensor
- **WHEN** se captura el dataset para recalibrar
- **THEN** el `DataFrame` usado por `recalibrate_predictor` y el `dataset_sha256` registrado en el linaje se derivan de una única lectura de bytes del archivo (`DatasetSnapshot`), nunca de dos lecturas independientes que podrían ver contenido distinto entre sí

#### Scenario: Cambio del dataset durante la captura aborta antes de registrar

- **GIVEN** que el archivo del dataset cambia (según `(mtime, size)`) entre el momento en que empieza y termina de leerse para construir la instantánea
- **WHEN** se solicita la recalibración
- **THEN** la solicitud falla explícitamente y no se registra ningún predictor sucesor ni evento de linaje con esa instantánea potencialmente inconsistente
...
**`dataset_sha256`, distinto de `dataset_fingerprint` (T-01):** `dataset_fingerprint` (`(mtime, size)`, ver `data_ingestion.storage.get_dataset_fingerprint`) sigue siendo únicamente la clave económica de invalidación de caché/detección de cambio (y la que usa `execute_configured_pipeline` para el caché de modelo por sensor) — no identifica el contenido: dos archivos distintos pueden compartir tamaño y fecha de modificación. `dataset_sha256` es la provenance persistente y verificable del dataset realmente usado en cada recalibración nueva: el SHA-256 (hexadecimal en minúsculas, 64 caracteres) del contenido binario exacto del archivo.

**Instantánea consistente dataset↔hash (T-01, microajuste R2 — 2026-09-06):** calcular `dataset_sha256` reabriendo el archivo por separado del `DataFrame` ya cargado permitiría, en teoría, que ambos correspondieran a contenidos distintos si el archivo cambia entre una lectura y la otra. `data_ingestion.storage.DatasetSnapshot` (`load_dataset_snapshot`) cierra esa brecha: lee los bytes del archivo una única vez, verifica que `(mtime, size)` no haya cambiado entre el inicio y el fin de esa lectura (si cambió, aborta con `RuntimeError` en vez de devolver una instantánea potencialmente inconsistente), calcula el SHA-256 incrementalmente sobre esos mismos bytes ya capturados, y construye el `DataFrame` desde ese mismo contenido (`pd.read_parquet` sobre un buffer en memoria, no una segunda lectura del archivo). `DatasetSnapshot.cache_fingerprint` es exactamente el mismo `(mtime, size)` de `get_dataset_fingerprint` — sigue siendo solo la clave de caché, `dataset_sha256` no la reemplaza. `backend/app/pipeline.py::load_dataset_snapshot_or_raise` (usado únicamente por `POST /recalibrate/{sensor_id}`; `POST /forecast/{sensor_id}/run` sigue usando `load_dataset_or_raise`, sin cambios, porque no necesita `dataset_sha256`) traduce ese `RuntimeError` a `ValueError` para que el router lo mapee a HTTP 400, sin llegar a `register_recalibrated_model`.
...
**Consistencia parámetro↔artefacto de `lineage_version` (T-01, microajuste R1 — 2026-09-06):** el parámetro MLflow `lineage_version` (cuando está presente) debe ser un entero soportado y coincidir exactamente con el `lineage_version` contenido en el artefacto; un artefacto V1 histórico genuino puede no tener el parámetro, pero su ausencia solo es válida para un artefacto efectivamente `LINEAGE_VERSION_1` — nunca se interpreta como V1 cuando el artefacto es de una versión posterior. Cualquier cruce incompatible (parámetro V2 + artefacto V1, parámetro V1 + artefacto V2, parámetro ausente + artefacto V2, o un parámetro que ni siquiera es un entero soportado) falla explícitamente. Además, `RecalibrationLineage.from_dict` normaliza toda estructura de artefacto inválida (el artefacto no es un objeto JSON, `{}`, una lista, un escalar, `feedback_references` no es una lista, una referencia mal formada o con campos ausentes/inesperados) a `LineageValidationError` — nunca propaga `TypeError`/`KeyError`/`AttributeError` sin envolver; `_load_lineage_for_version` agrega una red de seguridad equivalente por si algo se le escapara.
...
`POST /recalibrate/{sensor_id}` (`openspec/specs/alerting-ui/spec.md`) construye el evento únicamente con las correcciones nuevas devueltas por `recalibrate_predictor` (`dates`), nunca con todo `feedback_log`, con `lineage_version=CURRENT_LINEAGE_VERSION` y `dataset_sha256` de la misma instantánea (`DatasetSnapshot`) usada para recalibrar, y no registra ningún evento cuando `recalibrate_predictor`, la captura del dataset, o la validación del linaje lanzan `ValueError`/`RuntimeError`/`LineageValidationError` (la respuesta HTTP 400 se produce antes de llegar al registro). Testeado en `tests/test_recalibration_lineage.py`, `tests/test_model_registry.py`, `tests/test_storage.py` (instantánea dataset↔hash, aborto ante sustitución del archivo durante la lectura) y `backend/tests/test_recalibration.py` (ciclo completo A→B→C con forecast real avanzando el dataset entre ciclos verificando `dataset_sha256` contra el contenido real, y aborto sin registro cuando la captura del dataset falla).
