...
def to_daily(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_columns = [c for c in working.select_dtypes(include="number").columns if c != "_date"]
    ...
    if PROVENANCE_COLUMN in df.columns:
        provenance_by_day = working.groupby("_date")[PROVENANCE_COLUMN].first().reset_index()
        provenance_by_day = provenance_by_day.rename(columns={"_date": TIMESTAMP_COLUMN})
        provenance_by_day[TIMESTAMP_COLUMN] = pd.to_datetime(provenance_by_day[TIMESTAMP_COLUMN])
