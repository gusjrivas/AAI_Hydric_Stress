...
def validate_state(change):
    ...
    for event in change['state_events']:
        ...
        require(all(event.get(k) for k in ('from', 'to', 'actor', 'timestamp_utc', 'reason', 'evidence')), 'incomplete event')
    ...
    if status in approved_states or change['approved_for_implementation'] or change['scientific_execution_authorized']:
        approval = change['approval']
        require(isinstance(approval, dict), 'approval record required')
        require(all(approval.get(k) for k in ('actor', 'timestamp_utc', 'scope', 'source')), 'incomplete approval')
    if status == 'NOT_APPLICABLE':
        require(change['conditional'] is True, 'only conditional changes can be inapplicable')
        require(bool(change.get('applicability_decision')), 'audited applicability decision required')
    if status in {'PASS', 'NOT_APPLICABLE'}:
        ...
        require(bool(audit.get('snapshot')) and bool(audit.get('evidence')), 'audit identity/evidence required')


class GovernanceTests(unittest.TestCase):
    def setUp(self):
        self.requirements = json.loads((GOV / "requirements.json").read_text(encoding="utf-8"))["requirements"]
    ...
    def test_spec_matrix_tasks_are_consistent(self):
        ...
        for r in self.requirements:
            ...
            tasks = (ROOT / "openspec/changes" / r["change"] / "tasks.md").read_text(encoding="utf-8")
    ...
    def test_change_contracts_and_no_authorization(self):
        ...
        for c in self.changes:
            ...
            for heading in ("Dependencias", "Archivos autorizados", "Archivos excluidos", "Tareas y pruebas", "Evidencia esperada", "Aceptación", "Bloqueo", "Riesgos", "Rollback"):
    ...
    def event(start, end):
        ...
        return dict(from_=start, to=end, actor='fixture', timestamp_utc='2000-01-01T00:00:00Z',
                    reason='fixture', evidence='fixture', **{'from': start})

    def test_accepts_documented_approval_without_scientific_permission(self):
        record = copy.deepcopy(self.changes[0])
        record.update(status='APPROVED', approved_for_implementation=True,
    ...
    def test_not_applicable_is_not_a_shortcut_for_core_changes(self):
        record = copy.deepcopy(self.changes[0])
        record.update(status='NOT_APPLICABLE', state_events=[self.event('PLANNED', 'NOT_APPLICABLE')])
        with self.assertRaises(ValueError):
            validate_state(record)
    ...
    def test_pass_requires_independent_audit_evidence(self):
        ...
        record.update(status='PASS', approved_for_implementation=True,
                      ...
                      state_events=[self.event('PLANNED', 'APPROVED'), self.event('APPROVED', 'IN_PROGRESS'),
                                    self.event('IN_PROGRESS', 'REVIEW'), self.event('REVIEW', 'PASS')])
        with self.assertRaises(ValueError):
            validate_state(record)
    ...
    def test_dependencies_and_terminal_gate(self):
        ...
        for key in ('sc-07-aux-regression', 'sc-08-aux-hitl', 'sc-09-aux-anomalies', 'sc-10-aux-robustness'):
    ...
    def test_agents_toml_fields_models_and_permissions(self):
        ...
        for path in (ROOT / ".codex/agents").glob("*.toml"):
            ...
            found[a["name"]] = tuple(a[k] for k in ("model", "model_reasoning_effort", "sandbox_mode"))
